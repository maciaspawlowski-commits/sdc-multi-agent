...
# every pod restart re-pulls 6.3 GB from Docker Hub, which can wedge cri-dockerd
# and leave Ollama stuck in PodInitializing for 30+ minutes on a slow network.
#
...
# daemon, then starts everything else. Idempotent — safe to run any number of
# times.
...
if ($mkStatus -ne 'Running') {
    Write-Host "  minikube not running — starting it (this may take a minute)..." -ForegroundColor Yellow
...
Write-Host "`n[2/5] Ensuring ollama/ollama:cached-local tag exists in minikube..." -ForegroundColor Cyan
...
# ── 3. Apply manifests + ensure pods are Ready ───────────────────────────────
...
Write-Host ""
Write-Host "To start LangGraph Studio (visual graph view):"
